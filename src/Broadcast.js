...
class Broadcast {
  ...
  _removeOneListener(type, listener) {
    if (typeof listener !== "function")
      throw new TypeError(
        `listener should be Function, Received ${typeof listener}!`
      );
  ...
  emit(type) {
    ...
    if (typeof handlers === "function") {
      ...
      throw new TypeError(
        `listeners in ${type} is either undefined function nor array !`
      );
    }
  }
  getListeners(type) {
...
function _addListener(target, type, listener, prepend) {
  ...
  if (typeof listener !== "function") {
    throw new TypeError(
      `listener expect to be of type Function, but received ${typeof listener}!`
    );
  }
  if (events === undefined) {
    ...
    if (events[TYPES.addListener] !== undefined) {
      ...
      target.emit(TYPES.addListener, type, listener);
      //XXX: Is it necessary in event.js: Re-assign `events` because a addListener handler could have caused the this._events to be assigned to a new object
...
function _onceWrap(target, type, listener) {
  ...
  let wrapper = { fired: false, target, type, listener, wrapFn: undefined };
